...
import { lightRGBs, darkRGBs } from "@/components/ui/gluestack-ui-provider/config";
...
const formatRGBsToColors = (colors: Record<string, string>) => {
  const colorsArray = Object.entries(colors).map(([key, value])=> [key.replace('--color-', '').replaceAll('-', '_'), `rgb(${value})`])
